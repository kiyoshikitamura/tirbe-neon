-- Server-authoritative patrol rewards using the current quest master.
...
begin
  if p_xp_amount < 0 then raise exception 'XP amount must not be negative'; end if;
...
begin
  ...
  values (
    ...
    'クエスト報酬: ' || v_patrol.course_name, 'UNCLAIMED', now(), now() + interval '24 hours'
  );
  ...
  loop
    ...
    if v_item_id is not null and random() <= least(v_item_chance, 1) then
      ...
      values (
        ...
        'クエストドロップ: ' || v_patrol.course_name, 'UNCLAIMED', now(), now() + interval '24 hours'
      );
      v_awarded_items := v_awarded_items || jsonb_build_array(jsonb_build_object('item_id', v_item_id, 'quantity', v_item_quantity));
    end if;
  end loop;
  ...
  set status = 'COMPLETED', battle_resolved = true, rewards_accrued = v_reward_snapshot
...
-- Old snapshots may contain encounter flags created from a fallback chance
-- despite having no linked NPC master. Release those patrols for normal claim.
update public.user_patrols patrol
set has_battle_event = false, battle_resolved = true, battle_result = null
