...
type Unit = InputUnit & { hp: number; cooldowns: Record<string, number>; statuses: Partial<Record<Status, number>>; rawDamage: number };
...
export function resolveBattle(seed: number, tactic: Tactic, maxRounds: number, player: InputUnit[], enemy: InputUnit[]) {
  ...
  const makeUnit = (unit: InputUnit): Unit => ({ ...unit, hp: unit.stats.hp, statuses: {}, rawDamage: 0, cooldowns: Object.fromEntries(unit.skills.map((skill) => [skill.id, Math.max(0, skill.initialCooldown ?? 0)])) });
  ...
  while (round < maxRounds && live(players).length && live(enemies).length) {
    round++;
    const order = [...live(players), ...live(enemies)].sort((a, b) => b.stats.spd - a.stats.spd || (a.team === b.team ? a.id.localeCompare(b.id) : a.team === "PLAYER" ? -1 : 1));
    for (const actor of order) {
      ...
      if (actor.statuses.STUN) {
        ...
        for (const target of targets.filter(Boolean)) {
          ...
          const mitigation = target.stats.def / (target.stats.def + 27000);
          ...
          const amount = Math.max(1, Math.floor(actor.stats.atk * chosen.powerPercent / 100 * (1 - mitigation) * attribute * (critical ? 1.5 : 1) * (0.95 + rng.next() * 0.1)));
