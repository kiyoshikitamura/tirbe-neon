...
export default function SetupView() {
  ...
  return (
    ...
    >
      {!showRegistration ? (
        <div className="setup-box setup-intro-box auth-box border-cyan-glow background-black-80 shadow-cyan-20">
          <TutorialNavigator message="ようこそ。まずはあなたの名前を教えて。" />
          <button
            className="claim-reward-btn setup-primary-action active-scale-effect"
            onClick={() => {
              handleFirstUserInteraction();
        ...
        <div className="flex-col-gap-3 text-left">
          <div>
            ...
            <input
              ...
              maxLength={8}
              className="setup-name-input bg-black border-subtle text-white rounded outline-none width-100"
            />
          </div>

          {setupGiftCode ? (
            <div className="setup-invite-status" role="status">招待コードを受け取りました。</div>
          ) : (
            <details className="setup-invite-details">
              <summary>招待コードをお持ちの方</summary>
              <label htmlFor="setup-invite-code">招待コード（任意）</label>
              <input
                ...
                maxLength={8}
                className="setup-invite-input bg-black border-subtle text-white rounded outline-none width-100"
              />
            </details>
          )}
          ...
          <button
            ...
            className="claim-reward-btn setup-primary-action mt-4 font-weight-bold flex-row-center-spinner active-scale-effect justify-center"
          >
