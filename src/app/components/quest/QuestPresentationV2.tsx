  ...
  return <HubPage className="patrol-container quest-v2-shell" title="クエスト" hideVisualHeader>
    <div className="quest-v2-content" style={{ "--quest-state-background": `url(${bgImage})` } as React.CSSProperties}>
      ...
      {!selectionVisible && selectedPatrol && <div className="quest-v2-progress-list">{[selectedPatrol].map((patrol: any) => {
        ...
        if (battleRequired) return <section className="tutorial-quest-wire quest-v2-state-surface" data-quest-state="BATTLE_READY" key={patrol.id}><div className={`tutorial-wire-encounter ${unresolvedBattle ? "is-ready" : ""}`}><div className="tutorial-wire-glitch" aria-hidden="true">⚔</div><h2>バトル発生</h2><small>{course?.name || "クエスト"}</small><OutlawButton variant="primary" fullWidth disabled={!unresolvedBattle || battleStartingId === patrol.id || game.battleEncounterLocked} isLoading={battleStartingId === patrol.id} loadingLabel="バトル準備中…" onClick={() => void startBattle(patrol, npc)}>{unresolvedBattle ? "バトルへ" : "遭遇情報を同期中…"}</OutlawButton><OutlawButton className="quest-v2-selection-return" fullWidth onClick={() => setShowSelection(true)}>別のクエストへ派遣</OutlawButton></div></section>;
