...
export default function QuestPresentationV2() {
  ...
  return <HubPage className="patrol-container quest-v2-shell" eyebrow="クエスト" title="クエスト" description={`${townName}・${activeCourse ? difficulty(activeCourse.level_type) : "クエスト選択"}`}>
    <div className="quest-v2-content" style={{ "--quest-state-background": `url(${bgImage})` } as React.CSSProperties}>
      {(game.activePatrols || []).length === 0 && <><section className="quest-v2-identity" style={{ backgroundImage: `url(${bgImage})` }}>
      ...
      {activeCourse && <section className="quest-v2-brief">
        ...
        <OutlawButton variant="primary" fullWidth disabled={game.dispatchLoading || !game.selectedCourse || !game.selectedPatrolMember || Number(game.activePatrols?.length || 0) >= 5} isLoading={game.dispatchLoading || pendingAction === "DISPATCH"} loadingLabel="派遣準備中…" onClick={() => { setPendingAction("DISPATCH"); void guarded(() => game.handleStartPatrol()); }}>{townName}へ派遣する</OutlawButton>
      </section>}</>}

      {(game.activePatrols || []).length > 0 && <div className="quest-v2-progress-list">{(game.activePatrols || []).map((patrol: any) => {
