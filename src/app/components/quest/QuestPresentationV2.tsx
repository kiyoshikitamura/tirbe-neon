...
export default function QuestPresentationV2() {
  ...
  return <HubPage className="patrol-container quest-v2-shell" title="クエスト" hideVisualHeader>
    ...
    {game.showPatrolRewardModal && game.lastPatrolRewards && game.battleState === null && <CanonicalDialog title="クエスト結果" onClose={() => { game.setShowPatrolRewardModal(false); game.setLastPatrolRewards(null); }} actions={[{ label: "閉じる", semantic: "primary", onClick: () => { game.setShowPatrolRewardModal(false); game.setLastPatrolRewards(null); } }]}><div className="quest-v2-result"><span>{game.lastPatrolRewards.battleVictory ? "勝利" : "帰還完了"}</span><strong>{game.lastPatrolRewards.courseName}</strong>{game.lastPatrolRewards.matchBonusApplied && <p className="quest-v2-result-bonus">地元一致ボーナス適用</p>}<div className="quest-v2-rewards">{Number(game.lastPatrolRewards.totalCash || 0) > 0 && <RewardIcon itemId="CASH" label="CASH" quantity={Number(game.lastPatrolRewards.totalCash)} />}{Number(game.lastPatrolRewards.totalXp || 0) > 0 && <RewardIcon itemId="PLAYER_XP" label="PLAYER XP" quantity={Number(game.lastPatrolRewards.totalXp)} />}{game.lastPatrolRewards.dropItemName && Number(game.lastPatrolRewards.dropItemQty || 0) > 0 && <RewardIcon itemId={String(game.lastPatrolRewards.dropItemName)} label={canonicalItemName(String(game.lastPatrolRewards.dropItemName))} quantity={Number(game.lastPatrolRewards.dropItemQty)} />}</div>{game.lastPatrolRewards.levelUpMessage && <p>{game.lastPatrolRewards.levelUpMessage}</p>}</div></CanonicalDialog>}
  </HubPage>;
}
