...
function statusClass(status?: string) { return `is-${(status || "NOT_READY").toLowerCase().replace("_", "-")}`; }
...
export default function KpiDashboardV2() {
  ...
  const phases = [
    ...
    { label: "Product / Community", status: "NOT_READY", note: "Continuity definition pending" },
    { label: "Monetization", status: "UNAVAILABLE", note: "Payment CLOSED" },
  ];
  const retentionSummary: Metric | undefined = data?.retention?.cohorts?.flatMap((c: any) => c.days || []).find((d: any) => d.day === 1 && d.observation_status === "complete");
  const communityMetric: Metric = { metric_key: "community.effective_active_guild", numerator: null, denominator: null, value: latestCommunity?.effective_active_guild_count ?? null, target: 18, status: latestCommunity ? (latestCommunity.effective_active_guild_count >= 18 ? "PASS" : "FAIL") : "NOT_READY", as_of: latestCommunity?.date, observation_status: latestCommunity ? "complete" : "incomplete" };
  return <main className="kpi-shell v2-shell">
    ...
    <Section eyebrow="02 / CURRENT RELEASE GATE" title="Current Release Gate"><div className="v2-card-grid">
      ...
      <MetricCard label="Retention D1" metric={retentionSummary} definition="Game Start cohort / JST Classic Retention" />
      <MetricCard label="Community" metric={communityMetric} definition="Effective Active Guild: active≥3 AND chat active≥2" />
    </div></Section>
