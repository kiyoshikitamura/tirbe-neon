...
共通UIデザインは`ui_design_system.md`を正本とする。本書のUI記述は機能と画面固有情報の根拠として使用し、色、文字、ボタン、モーダル、ロード方式が異なる場合は共通UI正本を優先する。

本仕様書は、既存タイトル `code:wirth-dawn` の技術・機能アセットをベースに、ギルド、レイド、GvG（毎日変動・シーズン制）、単独PvP、固定属性、専用装備・専用スキル、およびシナリオエンジンを統合した『TRIBE NEON』の全体設計図です。
...
erDiagram
    USERS {
        ...
        int pvp_points "PvPポイント"
        timestamp last_stamina_recovery_at
        timestamp last_pvp_point_recovery_at
    ...
    BASES {
        uuid id PK
        string name "新宿、渋谷、池袋、六本木、秋葉原、川崎、横浜"
        string description
    ...
    USER_EQUIPMENTS {
        ...
        int exp
        int plus_val "限界突破+値"
        jsonb unlocked_options "限界突破で解放済みの固定追加オプション"
...
| `locations` | **`bases`** | 新宿、渋谷、池袋、六本木、秋葉原、川崎、横浜の7世界拠点。内部テーブル名`bases`は互換性のため維持する。 |
...
| `user_skills` | **`user_skills`** | アカウント単位のスキルID解放・限界突破値を管理する（`ユーザーID × スキルID`を一意）。レアリティ、専用制限、クールダウン・対象・効果はスキルマスタで管理し、キャラクターへの装備状態は別の装備テーブルで管理する。共有APは保持しない。 |
...
4. **クエスト（派遣）画面 (Quest / シノギ)**
   - 派遣先（新宿〜横浜の7箇所）を選択後、コース（初・中・上級の全21コース）を選び、所持キャラクター1体をスタミナ消費で派遣（シノギ）します。同時派遣は最大5枠（最大5体）とし、通常バトルの出撃編成および友達リーダーとは完全に分離します。
...
5. **マップ画面 (Map)**
   - 現代東京圏の7拠点（新宿、渋谷、池袋、六本木、秋葉原、川崎、横浜）を選択できる、モバイル向け横方向拠点ナビゲーションを使用する。1枚の鳥瞰マップへ全拠点を詰め込まない。
   - 各拠点をタップすると「支配ギルド名」「レイドボス出現状況」「占領ポイント数」を表示し、ワンタップで即座に移動が完了。所属拠点の概念はなく、移動は自由です。
6. **バトル画面 (Battle)**
   - 所持キャラクター最大5体と友達リーダー最大1体による、最大6体の完全オート進行・個別HP管理バトル画面（通常／レイド／PvP／GvG共用）。前衛・後衛の配置は設けない。
   - バトル開始前の`SETUP`（確認）フェーズでは、敵の属性・各パラメータ・総合力・装備スキルを確認し、攻撃側の作戦を5種類から選択する。キャラクター、装備、スキルはキャラクターページで事前に編集する。
   - バトル中（`PLAYING`）は作戦変更不可の完全オート進行で、タイムライン順のスキル自動発動、ターゲットレーザーラインや発動カットイン、被弾振動、個別HPメーター直上ポップアップなどの演出を経て結果まで見届けます。挑戦用APは戦闘外リソースとして扱う。
...
7. **キャラクター画面 (CharacterTab)**
   - **画面構成**: 
     ...
     - **下部詳細パネル**: 
       - `ステータス・育成`：レベルアップ、覚醒限界突破（表示名「覚醒の書」。internal ID `LAW_OF_STRIFE`）、ステータスグリッド。必要冊数・CASH併用はPROVISIONAL。
       - `スキルデッキ`：覚醒により3〜6枠となるスキル装備スロット。戦闘中の使用可否はスキル固有クールダウンで判定する。
...
2. **バトル進行フック (`useBattle.ts`)**:
   - 完全オートバトルにおける複雑な状態遷移、NPC作戦AI、個別HP、スキル固有クールダウン・効果、および被ダメージ時・回復時のアニメーション演出制御をカプセル化して提供する独立カスタムフックです。
...
- 得意スキルボーナスは初期実装では設けない。ロール適性は自動装着・AI評価にのみ用いる。
- **専用装備・専用スキル**: 該当キャラクターのみ装備可能。専用装備は限定的な固定固有効果、専用スキルは高い効果量・固有の追加効果で価値を持たせる。
- **ギルド属性防衛ボーナス**: 防衛側デッキのキャラクターの属性がギルドのメインアライメント属性と一致する場合にパラメータ（HP/ATK/DEF）+20%、サブアライメントと一致する場合に+10%が適用。

### ② ギルドバトル（GvG：拠点縄張り争奪戦）
- GvGは、両ギルドの共通ギルドHPを主勝敗条件とし、個人の実与ダメージを貢献ポイントとして扱うハイブリッド方式とする。
- 固定のギルド支配ポイント加減算、日次拠点支配権、支配日数による順位決定は廃止する。
- 開催時刻、事前マッチング、NPCギルド戦、ランク、対戦・シーズン報酬は`spec_battle_system.md`を優先する。
...
- **防衛デッキの作戦設定および戦闘AI**:
  - 防衛デッキ（`pvp_defense_decks`）のメンバーを登録する際、防衛時AIに適用する作戦を保存できます。作戦は「攻撃優先」「回復優先」「スキル優先」「バランス」「弱点集中」の5種類とし、防衛デッキの登録・更新時のみ変更可能です。
