...
try{
 ...
 assert(start.attempt_number===1&&start.cost===0,'First Raid attempt was not free');assert(start.guild_id_snapshot===null,'Unguilded Raid did not keep a null guild snapshot');
 ...
 const {data:ranks,error:rankError}=await p.client.rpc('get_raid_rankings',{p_instance_id:raids[0].id});if(rankError)throw rankError;assert(ranks.individual.some(r=>r.user_id===p.id),'Individual Raid ranking missing');assert(ranks.guild.length===0,'Unguilded contribution leaked into Guild ranking');
 ...
 console.log(JSON.stringify({projectRef:expected,status:'PASS',raidCount:raids.length,bases:raids.map(r=>r.baseId),replayId:start.replay_session_id,damage:{raw:result.rawDamage,applied:result.appliedDamage},checks:['unguilded start','two locations','24h instances','server result','guild null snapshot','individual ranking','retry idempotency','consumer finalize denied','legacy damage denied']},null,2));
