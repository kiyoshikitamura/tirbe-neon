...
async function resolve(client, replaySessionId) {
  let { data, error } = await client.functions.invoke("resolve-battle", { body: { replaySessionId } });
  if (error) {
    const retry = await client.functions.invoke("resolve-battle", { body: { replaySessionId } });
    data = retry.data;
    error = retry.error;
  }
  if (error) {
    const detail = await error.context?.json?.().catch(() => null);
    throw new Error(`resolve-battle failed: ${JSON.stringify(detail || error.message)}`);
  }
  return data;
}
...
try {
  ...
  const { error: gachaError } = await player.client.rpc("execute_character_gacha", {
    p_user_id: player.id, p_gacha_id: "CHAR_NORMAL", p_pull_count: 1, p_currency_type: "cash",
  });
  ...
  assert(recommendations?.some((guild) => guild.guild_id === guildId), "Active open Guild was not recommended.");
  await recordClientEvent(player.client, "guild_recommendation_impression", "raid", null, guildId);
  await recordClientEvent(player.client, "guild_detail_view", "raid", "recommended_guild", guildId);
  ...
  console.log(JSON.stringify({
    ...
    checks: ["Lv5 bridge", "two 24h Raid locations", "Guild snapshot", "Guild ranking", "retry idempotency", "event allowlist"],
  }, null, 2));
} finally {
